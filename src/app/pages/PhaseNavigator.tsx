...
const PHASE_LIST: { key: GamePhase; label: string }[] = [
...
const CORE_STRATAGEMS: CoreStratagem[] = [
  ...
  },
  {
    ...
    target: 'One unit from your army that is within 24" of that enemy unit and that would be eligible to shoot.',
    effect: 'Your unit can shoot that enemy unit as if it were your Shooting phase, but its models can only make Hit rolls on unmodified 6s.',
  ...
  },
  {
    ...
    target: 'One unit from your army that is within 6" of that enemy unit and would be eligible to declare a charge in your turn.',
  ...
  },
  {
    ...
    cp: '1CP',
    type: 'Core Stratagem',
    when: 'Any phase, just after you fail a Battle-shock test taken for a unit from your army.',
  ...
  },
  {
    ...
    effect: 'Your unit can be set up on the battlefield as described in the rules for the ability that placed it into Reserves.',
    phases: ['movement'],
  },
  {
    name: 'Smokescreen',
  ...
  },
  {
    ...
    target: 'One VEHICLE unit from your army that has not declared a charge this phase.',
    effect: 'In this phase, after your unit ends a Charge move, select one enemy unit within Engagement Range of it, then roll a number of D6 equal to the Toughness characteristic of your VEHICLE model that is closest to that enemy unit. For each 5+, that enemy unit suffers 1 mortal wound.',
    phases: ['charge'],
  },
];
...
function StratagemCard({ strat, badge }: {
  ...
  return (
    <button
      ...
      className="w-full text-left p-4 bg-[#f5efe6] border-2 border-[#b8860b]/30 rounded-lg
                 hover:border-[#b8860b]/60 transition-all"
    >
      <div className="flex items-start justify-between gap-2 mb-1">
        <h4 className="text-sm font-bold text-[#2c2416]">{strat.name}</h4>
        <div className="flex items-center gap-1.5 shrink-0">
          {badge && (
            <span className="text-[10px] px-1.5 py-0.5 bg-[#8b7355] text-[#faf6f0] rounded font-semibold uppercase">
              {badge}
            </span>
          )}
          {strat.cp && (
            <span className="text-xs px-2 py-0.5 bg-[#b8860b]/15 text-[#b8860b] font-bold rounded border border-[#b8860b]/30">
              {strat.cp}
            </span>
          )}
        </div>
      ...
      {strat.type && (
        <p className="text-[10px] text-[#8b7355] uppercase tracking-wider mb-1">{strat.type}</p>
      )}
      {expanded && (
        <div className="mt-2 space-y-1.5 text-xs text-[#5c4a32]">
          {strat.when && (
            <div><span className="font-semibold text-[#8b7355]">When: </span>{strat.when}</div>
          )}
          {strat.target && (
            <div><span className="font-semibold text-[#8b7355]">Target: </span>{strat.target}</div>
          )}
          {strat.effect && (
            <div><span className="font-semibold text-[#8b7355]">Effect: </span>{strat.effect}</div>
          )}
          {strat.restrictions && (
            <div><span className="font-semibold text-[#8b7355]">Restrictions: </span>{strat.restrictions}</div>
          )}
        </div>
      )}
      {!expanded && strat.effect && (
        <p className="text-xs text-[#8b7355] mt-1 line-clamp-2">{strat.effect}</p>
      )}
    </button>
  );
}
...
function UnitAbilityRow({ unitName, abilityName, abilityText }: {
  ...
  return (
    <button
      onClick={() => setExpanded(!expanded)}
      className="w-full text-left px-3 py-2 bg-[#f5efe6] border border-[#d4c5a9] rounded
                 hover:border-[#b8860b]/40 transition-all"
    >
      <div className="flex items-center gap-2">
        <span className="text-xs font-semibold text-[#b8860b] shrink-0">{unitName}</span>
        <span className="text-xs text-[#2c2416]">{abilityName}</span>
      </div>
      {expanded && (
        <p className="text-xs text-[#5c4a32] mt-1 leading-relaxed">{abilityText}</p>
      )}
    </button>
  );
}
...
export default function PhaseNavigator() {
  ...
  // Filter detachment stratagems by phase
  ...
  // Filter core stratagems by phase
  const coreStratagems = useMemo(() => {
    return CORE_STRATAGEMS.filter(s => s.phases.includes(activePhase));
  }, [activePhase]);

  // Filter unit abilities by phase
  ...
  if (!factionId) {
    return (
      <div className="min-h-screen bg-[#faf6f0] flex flex-col items-center justify-center px-4">
        <p className="text-[#8b7355] mb-4">Set up your army first to use Battle Aid.</p>
        <button
  ...
  return (
    <div className="min-h-screen bg-[#faf6f0] flex flex-col pb-24">
      {/* Header */}
      <div className="px-4 pt-6 pb-3">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h1 className="font-serif text-xl font-bold text-[#2c2416]">Battle Aid</h1>
            {detachmentName && (
              <p className="text-xs text-[#8b7355]">{detachmentName}</p>
            )}
          </div>
        </div>
            ...
            <button
              ...
              className={`whitespace-nowrap px-4 py-2 rounded-full text-sm font-medium border transition-all shrink-0 ${
                activePhase === phase.key
                  ? 'bg-[#b8860b] text-[#faf6f0] border-[#b8860b]'
                  : 'bg-[#f5efe6] text-[#8b7355] border-[#d4c5a9] hover:border-[#b8860b]/50'
              }`}
            >
              {phase.label}
            </button>
      ...
      <div className="flex-1 px-4 space-y-6 mt-4">
        ...
        {detachmentStratagems.length > 0 && (
          <section>
            <h3 className="text-xs font-semibold text-[#8b7355] uppercase tracking-wider mb-2">
              Your Stratagems
            </h3>
            <div className="space-y-2">
              {detachmentStratagems.map((strat, idx) => (
                <StratagemCard key={`det-${idx}`} strat={strat} />
              ))}
            </div>
          </section>
        )}
          ...
          <section>
            ...
            <div className="space-y-2">
              {coreStratagems.map((strat, idx) => (
                <StratagemCard key={`core-${idx}`} strat={strat} badge="Core" />
              ))}
            </div>
          </section>
        )}
        ...
        {detachmentStratagems.length === 0 && coreStratagems.length === 0 && unitAbilities.length === 0 && (
          <div className="text-center py-12">
            <p className="text-[#8b7355] text-sm">No content for this phase.</p>
          </div>
        )}
      </div>

      {/* Hide scrollbar for phase pills */}
      <style>{`
